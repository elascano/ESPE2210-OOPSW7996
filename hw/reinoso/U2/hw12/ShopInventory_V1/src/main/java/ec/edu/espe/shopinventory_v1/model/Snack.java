 ...
 */
public class Snack extends ProductWithVAT {
    ...
    public void createProduct() {
        try ( MongoClient client = MongoClients.create("mongodb+srv://JerlyReinoso:R1RNC@jerlyreinoso.vwcbpz1.mongodb.net/test")) {
            ...
            MongoCollection collection = db.getCollection("snack");
            System.out.print("Enter the id of the Snack: ");
            int snackId = scan.nextInt();
            System.out.print("Enter the name of the Snack: ");
            scan.nextLine();
            String snackName = scan.nextLine();
            System.out.print("Enter the quantity of the Snack: ");
            int snackQuantity = scan.nextInt();
            System.out.print("Enter the price of the Snack: ");
            float snackPrice = scan.nextFloat();
            Document newDocument = new Document("id", snackId).append("name", snackName).append("quantity", snackQuantity).append("pricePerUnit", snackPrice);
            ...
            snackPrice = Snack.computeVAT(snackPrice);
            Document newDocumentWithVAT = new Document("id", snackId).append("name", snackName).append("quantity", snackQuantity).append("pricePerUnit", snackPrice);
    ...
    public void updateProduct() {
        try ( MongoClient client = MongoClients.create("mongodb+srv://JerlyReinoso:R1RNC@jerlyreinoso.vwcbpz1.mongodb.net/test")) {
            MongoDatabase db = client.getDatabase("ShopInventory");
            MongoCollection collection = db.getCollection("Snack");
            System.out.println("Enter the ID of the Snack you want to Update:");
            int id = scan.nextInt();
