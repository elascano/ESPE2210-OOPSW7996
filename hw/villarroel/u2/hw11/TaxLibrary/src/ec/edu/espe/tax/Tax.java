...
public class Tax {
    ...
    public static float computeGreenTax(float cylinderSize, int carYear, String carType){
        ...
        antiquity=currentYear-carYear;

        if(cylinderSize<=1500){amountPerCylinder=0;}
        else if(cylinderSize>1500 && cylinderSize<=2000){amountPerCylinder=0.08F;}
        else if(cylinderSize>2000 && cylinderSize<=2500){amountPerCylinder=0.09F;}
        else if(cylinderSize>2500 && cylinderSize<=3000){amountPerCylinder=0.11F;}
        else if(cylinderSize>3000 && cylinderSize<=3500){amountPerCylinder=0.12F;}
        else if(cylinderSize>3500 && cylinderSize<=4000){amountPerCylinder=0.24F;}
        else if(cylinderSize>4000)                          {amountPerCylinder=0.35F;}

        if(carType.equalsIgnoreCase("gas") || carType.equalsIgnoreCase("diesel")){

            if(antiquity<5){percentagePerAntiquity=0;}
            else if(antiquity>=5 && antiquity <=10){percentagePerAntiquity=0.05F;}
            else if(antiquity>10 && antiquity <=15){percentagePerAntiquity=0.1F;}
            else if(antiquity>15 && antiquity <=20){percentagePerAntiquity=0.15F;}
            else if(antiquity>20){percentagePerAntiquity=0.2F;}

        }else if (carType.equalsIgnoreCase("hybrid")){
            percentagePerAntiquity=-0.2F;
        }
        greenTax = (cylinderSize-1500)*amountPerCylinder*(1+percentagePerAntiquity);
        return greenTax;
    }
     ...
     public static float computeIva(float base){
        float iva;
        iva = base * 0.12F;//this information must be centralized somewhere
        return iva;
    } 
